...
```{r}
data_loc <- here("Data")
igbp <- read.csv(here(data_loc, 'AreasIGBP.csv'), header = TRUE)
```

```{r}
igbp_fine <- read.csv(here(data_loc,"IGBP_Plotting_fine.csv"), header = TRUE)
...
# Define the order of categorical variables
custom_order <- c( "EBF", "DBF", "Mixed Forests", "Savannas", "Grassland", "Permanent Wetlands", "Barren")  # Replace with your desired order
...
```{r}
igbp_fine_sample <- read.csv(here(data_loc, "IGBP_Fine_SamplingDensity.csv"), header = TRUE)
...
for (i in 1:nrow(igbp_rough_sample)) {
  # Determine the highest absolute value between the two bars for y position
  max_val <- max(abs(igbp_rough_sample$SampleRate_BNF_sites[i]), abs(igbp_rough_sample$SampleRate_denit_sites[i]))
  # Determine y position and anchor based on the sign of the value
  if (max_val == abs(igbp_rough_sample$SampleRate_BNF_sites[i])) {
    y_pos <- ifelse(igbp_rough_sample$SampleRate_BNF_sites[i] > 0, igbp_rough_sample$SampleRate_BNF_sites[i] + 3, igbp_rough_sample$SampleRate_BNF_sites[i] - 3)
    y_anchor <- ifelse(igbp_rough_sample$SampleRate_BNF_sites[i] > 0, 'bottom', 'top')
  } else {
    y_pos <- ifelse(igbp_rough_sample$SampleRate_denit_sites[i] > 0, igbp_rough_sample$SampleRate_denit_sites[i] + 3, igbp_rough_sample$SampleRate_denit_sites[i] - 3)
    y_anchor <- ifelse(igbp_rough_sample$SampleRate_denit_sites[i] > 0, 'bottom', 'top')
  }
  
  annotations <- c(annotations, list(
    list(
      x = igbp_rough_sample$Biome_rough[i],
      y = y_pos,
