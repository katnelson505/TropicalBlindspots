...
tmp_sdm<-gam(PresAbs~ SoilN+SoilOC+NPP+MAT+Slope+SoilP+DtB1+AET+SoilMoisture+Seasonality,family = binomial, data=gam_data, select = TRUE, method="GCV.Cp")
...
##trying a GAM with only theoretically important variables 
```{r}
#try the Gam
...
#model only explains ~10% variation. 
...
```{r}
plot(tmp_sdm, shade = TRUE, pages = 1)
```

```{r}
library(gam)
stepwise_model <- step.gam(tmp_sdm)
...
cv_model <- train(response ~ ., data = your_data, method = "gam", trControl = train_control)
print(cv_model)
...
```{r}
vgm.gam <- variogram(Gam_pred ~ 1, locations = ~Lat + Long, data = gam_data, boundaries = c(5, 10, 15, 20, 25, 30))
vgm.gam
plot(vgm.gam)
```
...
```{r}
library(mgcv)
gam_data <- read.csv(here(pca_loc,"RandPoints_Ext_all_GAM.csv"), header = TRUE)
gam_data <- merge(gam_data, data, by = 0)
gam_data <- gam_data %>% dplyr::select(-c(Row.names))

tmp_sdm<-gam(PresAbs~ SoilN+SoilOC+NPP+MAT+Slope+SoilP+DtB1+AET+SoilMoisture,family = binomial, data=gam_data, select = TRUE, method="GCV.Cp")
#Then when you want to predict, you just:
...
library(mgcv)
gam_data <- read.csv(here(pca_loc,"RandPoints_Ext_all_GAM.csv"), header = TRUE)
gam_data <- merge(gam_data, raster_data, by = 0)
gam_data <- merge(gam_data, pc_scores, by = 0)
gam_data <- gam_data %>% dplyr::select(-c(Row.names))
colnames(gam_data)[which(names(gam_data) == "Dim.1")] <- "pc1"
#If run with above, data is already normalized and standardized 

```


MAT+Slope+SoilP+DtB1
PPT+AET+Seasonality+NPP+
SoilOC+SoilN
##GAM is overfitting when I add in AET 
```{r}
tmp_sdm<-gam(PresAbs~ SoilN+SoilOC+NPP+MAT+Slope+SoilP+DtB1,family = binomial, data=gam_data, select = TRUE, method="GCV.Cp")
#Then when you want to predict, you just:
Gam_pred <- predict.gam(tmp_sdm, newdata=data, type="response")
Gam_pred <- as.data.frame(Gam_pred)
#predictions for presence absence at each point based only on PPT and MAT
...
```{r}
gam_loc <- merge(Gam_pred, data2, by = 0)
gam_loc <- gam_loc %>% dplyr::select(c(Lat, Long, Gam_pred))
 gam_loc[gam_loc==0] <- NA
 gam_loc
 
 gam_loc<-gam_loc[complete.cases(gam_loc),]
#write.csv(gam_loc, file = "Gam_pred_noAET.csv")
```
